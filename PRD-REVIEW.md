...
O critério de aceite fiz que " O campo de título é limpo após a criação bem-sucedida
", porém não diz o que ocorre caso aconteça um erro.
...
[PRD-011] Não há requisitos para desfazer acidental de tarefas
...
Adicionar um requisito simples de: Ao clicar no checkbox, tarefa concluída, ao clicar novamente ele desfaz a conclusão da tarefa ou ou toggle
...
O critério de aceitação diz que "a mudança de estado persiste", mas não define o comportamento caso a chamada ao backend falhe. O usuário pode ver o checkbox marcado no frontend enquanto o a informação não foi  salva.
...
IA's podem retornar respostas fora do formato esperado, incompletas ou com algum outro problema. Sem a validação, o sistema pode tentar salvar uma resposta fora do padrão ou incompleta, causando erros no backend ou criando tarefas com títulos inválidos ou incompletos na lista de tarefas
