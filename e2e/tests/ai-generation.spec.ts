...
test.describe('Geração de Tarefas via IA', () => {
  ...
  test('[E2E-0012][RF-05] deve salvar automaticamente as subtarefas geradas na lista', async ({ taskPage, aiPage }) => {
  ...
  test('[E2E-0013][RF-05] deve exibir indicador de carregamento durante a geração', async ({ taskPage, aiPage }) => {
    // Intercepta a requisição e atrasa com setTimeout nativo (não depende do ciclo de vida do Playwright)
    await taskPage.page.route('**/ai/generate', async (route) => {
  ...
  test('[E2E-0014][RF-01][RF-05] deve diferenciar visualmente tarefas IA de manuais em lista mista', async ({ taskPage, aiPage }) => {
    ...
    await taskPage.createTask('Tarefa manual sem badge');
    ...
    expect(await taskPage.hasAiBadge('Tarefa manual sem badge')).toBe(false);

    // Quantidade de badges deve ser exatamente (total - 1 manual)
    const badgeCount = await taskPage.page.getByTestId('task-ai-badge').count();
    expect(badgeCount).toBe(totalTasks - 1);
  });

  test('[E2E-0015][RF-05] não deve gerar tarefas com objetivo vazio', async ({ taskPage, aiPage }) => {
    const countBefore = await taskPage.getTaskCount();
  ...
  test('[E2E-0016][RF-06][BUG-005] deve exibir erro ao usar API Key inválida', async ({ taskPage, aiPage }) => {
    ...
    const errorVisible = await taskPage.page.getByText(/inválid|erro|falhou/i).isVisible();
  ...
  test('[E2E-0017][RF-06][BUG-004] deve exibir mensagem de erro ao gerar sem API Key configurada', async ({
    ...
    await taskPage.page.waitForTimeout(2000);

    const errorVisible = await taskPage.page.getByText(/api key|chave|erro/i).isVisible();
    expect(errorVisible).toBe(true);
  });
});
